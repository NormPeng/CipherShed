...
namespace CipherShed
{
	...
	bool GraphicUserInterface::OnInit ()
	{
		...
		try
		{
			...
			if (SingleInstanceChecker->IsAnotherRunning())
			{
				...
				Gui->ShowInfo (_("CipherShed is already running."));
				return false;
			}
	...
	void GraphicUserInterface::OpenUserGuide (wxWindow *parent)
	{
		try
		{
			...
			docPath = StringConverter::ToWide (string (TC_TO_STRING (TC_RESOURCE_DIR)) + "/doc/CipherShed User Guide.pdf");
#elif defined (TC_WINDOWS)
			docPath += L"\\CipherShed User Guide.pdf";
#elif defined (TC_MACOSX)
			docPath += L"/../Resources/CipherShed User Guide.pdf";
#elif defined (TC_UNIX)
			docPath = L"/usr/share/ciphershed/doc/CipherShed User Guide.pdf";
#else
